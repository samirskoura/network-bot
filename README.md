...
needed for manual mode. Read `PERSONAL_HEADLINES.txt` for the exact steps.
...
headline and saved history of each individual Creative. A repeat of that
Creative's own previous wording is blocked, including changes only to spacing,
punctuation or Arabic diacritics. Another Creative's history does not block it.
Length and internal-name checks still apply.

The manual pool is considered independently for every eligible Creative. If ten
Creatives have never used its first suitable line, all ten may receive that line.
If one has already used it, that Creative takes its next acceptable option. The
OpenAI prompt follows the same sharing rule.
...
check against that Creative's own history. Used lines are never recycled when
a Creative exhausts its list. Add more lines before the next workflow run.

Existing per-Creative history, last-submitted headlines, pending reservations and
approval records are retained. Newly observed headlines are saved with their
Creative IDs. Legacy account-wide history and `blocked_headline_hashes.txt` are
retained as archived data, and are not used to block other Creatives. Unassigned
old history cannot identify which Creative used a headline; the bot cannot
reconstruct usage that was never recorded for a Creative. Keep the existing
encrypted state, or recover it through the supplied full-replacement workflow.
...
hash list. History recovered from encrypted state continues to prevent repeat
use on the same Creative. A new bot for another account should use a separate
...
after any OpenAI wait. Repeat checks use only that Creative's history;
...
issues may require corrections beyond a headline. This revision passed 46 offline
tests covering headline sharing, repeat prevention, status guards and OpenAI
recovery. The full package was checked for source
