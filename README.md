...
6. Repository Variables are optional for manual runs. The model falls back to
   `gpt-5.4-nano`; the Run workflow form supplies mode and target. If desired,
...
Replacing all files installs the supplied default headline pool and extra
blocked-hash list. History recovered from encrypted state continues to prevent
reuse of stored headlines. A new bot for another account should use a separate
...
after any OpenAI wait. Exact/near-duplicate and internal-name checks also remain
active. A request failure aborts generation before this cycle's headline edits.
